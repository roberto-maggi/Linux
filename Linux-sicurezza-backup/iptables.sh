...
# NETS
NFS='no'
NFS_ACCEPT=(192.168.56.0/24 )
NO_SPOOF_sNETS=( 169.254.0.0/16 172.16.0.0/12 127.0.0.0/8 192.168.0.0/24 192.168.10.0/24 224.0.0.0/4 240.0.0.0/5 0.0.0.0/8 )
NO_SPOOF_dNETS=( 240.0.0.0/5 224.0.0.0/4 0.0.0.0/8 239.255.255.0/24 255.255.255.255)

# VARS per FOWARDING  
FORWARDING='no'
IF_IN=eth1
IF_OUT=eth0
IP_OUT=$(ip --brief a s eth0 | awk '{print $3}'  | sed -e 's/\/24//g')
count=0

for NIC in $IFACES ; do 
sysctl net.ipv4.conf.$NIC.forwarding=1 > /dev/null
sysctl net.ipv6.conf.$NIC.forwarding=0 > /dev/null ; done
...
for BAN_sNETS in ${NO_SPOOF_sNETS[@]}; do
    $IPT -A INPUT -s $BAN_sNETS -j DROP ;
    done
for BAN_dNETS in ${NO_SPOOF_dNETS[@]}; do 
    $IPT -A INPUT -d $BAN_dNETS -j DROP ;
    done
let count++
echo -n " $count "
...
$IPT -N syn-flood
for NIC in $IFACES; do 
    $IPT -A INPUT -i $NIC -p tcp --syn -j syn-flood ; 
    done
...
# NFS 
if [[ $NFS == yes ]] ; then
    for NETS in ${NFS_ACCEPT[@]} ; do
        $IPT -A INPUT -s $NETS -m state --state NEW -p udp --dport 111 -j ACCEPT
        $IPT -A INPUT -s $NETS -m state --state NEW -p tcp --dport 111 -j ACCEPT
        $IPT -A INPUT -s $NETS -m state --state NEW -p tcp --dport 2049 -j ACCEPT
        $IPT -A INPUT -s $NETS -m state --state NEW -p tcp --dport 32803 -j ACCEPT
        $IPT -A INPUT -s $NETS -m state --state NEW -p udp --dport 32769 -j ACCEPT
        $IPT -A INPUT -s $NETS -m state --state NEW -p tcp --dport 892 -j ACCEPT
        $IPT -A INPUT -s $NETS -m state --state NEW -p udp --dport 892 -j ACCEPT
        $IPT -A INPUT -s $NETS -m state --state NEW -p tcp --dport 875 -j ACCEPT
        $IPT -A INPUT -s $NETS -m state --state NEW -p udp --dport 875 -j ACCEPT
        $IPT -A INPUT -s $NETS -m state --state NEW -p tcp --dport 662 -j ACCEPT
        $IPT -A INPUT -s $NETS -m state --state NEW -p udp --dport 662 -j ACCEPT ;
    done ;
fi
let count++
echo -n " $count "
